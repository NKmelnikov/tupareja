...
class UploadHelper {
    ...
    public $chunksFolder = '/wp-content/uploads';
    ...
    public function combineChunks($uploadDirectory, $name = null) {
        ...
        $targetFolder = $this->chunksFolder.date("Y").DIRECTORY_SEPARATOR.date("d.m");
        $totalParts = isset($_REQUEST['qqtotalparts']) ? (int)$_REQUEST['qqtotalparts'] : 1;

        $targetPath = join(DIRECTORY_SEPARATOR, array($uploadDirectory, date("Y"), date("d.m"), $name));
        ...
        if (!is_null($this->sizeLimit) && filesize($targetPath) > $this->sizeLimit) {
            ...
            return array("success" => false, "uuid" => $uuid, "path" => $target, "preventRetry" => true);
        }
    ...
    public function handleUpload($uploadDirectory, $name = null){
        ...
        # chunked upload
            ...
            $success = move_uploaded_file($_FILES[$this->inputName]['tmp_name'], $targetFolder);
            return array("success" => true, "uuid" => $uuid,"path" => $targetFolder);
        }
        else {
        # non-chunked upload
            $target = join(DIRECTORY_SEPARATOR, array($uploadDirectory, date("Y"),date("d.m"), $name));
            if ($target){
    ...
    public function handleDelete($uploadDirectory, $name=null)
    {
        ...
        $target = join(DIRECTORY_SEPARATOR, array($uploadDirectory, date("Y"),date("d.m"), $name));
        file_put_contents('/tmp/logs/$target.txt', print_r($target, true), FILE_APPEND);
        if (is_file($target)){
            unlink($target);
